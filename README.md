...
1. **Customer Mobile App** (Flutter) — submit and track locker requests, complete identity verification.
...
- No NoSQL — PostgreSQL is the relational source of truth.
...
| Layer | Technology |
|---|---|
| Customer App | Flutter, Dart, Material 3 |
...
│   ├── app/
│   │   ├── api/routes/      auth, customers, requests, verification, admin, audit, branches, notifications
...
│   └── lib/{core,models,services,screens,widgets}
├── docker-compose.yml
└── .env.example
```
...
- UUID primary keys are stored as portable `String(36)` so the same models
  run against PostgreSQL in production and SQLite in the test suite.
...
1. Bank staff call `POST /verification/{request_id}/generate` → backend
   ...
   of each in PostgreSQL for durable audit evidence.
...
cd customer-mobile
flutter pub get
# Android emulator reaches host localhost via 10.0.2.2 (already the default)
...
- Compliance & Audit center with KPİs, filters, and a full chronological timeline per request (`correlation_id`)
...
- Customer app: login, home, my locker, request access, dual-token verification, request tracking, notifications, profile
...
- **Audit security:** every privileged operation (login, state transition, verification attempt/success/failure, approval, rejection, completion) writes an append-only `audit_events` row.
- **Logging:** structured logs include timestamp, level, request ID, endpoint, and duration — never passwords, raw OTPs, or raw verification tokens.
...
## 15. 36-Hour Implementation Strategy (as executed)

**P0 (must work) — delivered:** auth, customer locker view, locker request
submission, admin request queue, dual-token verification, locker/request
state machines, PostgreSQL persistence, Redis token management, audit
logging, visual locker grid, end-to-end demo flow.

**P1 (high value) — delivered:** compliance dashboard with filters, business
metrics (analytics) page, notifications, branch/customer views, polished
admin and mobile UX.

**P2 (only if time remains):** advanced real-time infrastructure (WebSockets;
current implementation uses simple polling per Section 22's "prefer the
simplest reliable implementation" guidance), export functionality, richer
analytics.

Team parallelization followed the spec's four-pair model: backend
(FastAPI/Postgres/Redis), admin portal, customer app, and
DevSecOps/testing/integration — with the API contract (Section 17/18 of the
original brief) fixed first so frontend work could proceed independently.
...
- Customer mobile app ships the P0 flow (login, locker, request, dual-token verification, tracking, notifications, profile) with Material 3 styling; deeper mobile polish (rich empty states, offline caching, push notifications) is intentionally out of scope for the hackathon window.
...
- Regulatory data-retention policies for audit events.
...
1. **Customer (0:00–1:00):** Log in as `customer@demo.bank` → Home shows
   Locker `A1`, status `OCCUPIED` → tap **Request Access** → submit.
